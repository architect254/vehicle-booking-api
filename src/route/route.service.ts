...
export class RouteService {
  ...
  async create(
    ...
    return `Route ${route.name} created successfuly`;
  }
  ...
  async update(
    ...
    return `Route ${route.name} updated successfuly`;
  }
  ...
  async drop(id): Promise<string> {
    ...
    if (!deletedRoute) {
      const errorMessage = `Failed to delete route:${deletedRoute.name}`;
      throw new InternalServerErrorException(errorMessage);
    }

    return `Route ${deletedRoute.name} deleted successfuly`;
  }
